...
-- Generate login credentials for role-based users (ADMIN, MANAGER, EMPLOYEE)
-- not covered by static seed rows.
-- Login username is Employee.email, default password is Password123!.
INSERT INTO User_Credentials (
    CredentialId,
    EmployeeId,
    PasswordHash,
...
SELECT
    base.MaxCredentialId + ROW_NUMBER() OVER (ORDER BY e.id) AS CredentialId,
    e.id AS EmployeeId,
    '{noop}Password123!' AS PasswordHash,
    NULL AS LastLogin,
    'Y' AS IsActive,
    0 AS FailedLoginAttempts,
    NULL AS LockedUntil,
    NULL AS PasswordResetToken,
    NULL AS PasswordResetExpiry,
    NOW() AS CreatedAt,
    NOW() AS UpdatedAt
FROM Employee e
LEFT JOIN User_Credentials uc ON uc.EmployeeId = e.id
CROSS JOIN (SELECT COALESCE(MAX(CredentialId), 0) AS MaxCredentialId FROM User_Credentials) base
WHERE uc.EmployeeId IS NULL
    AND EXISTS (
            SELECT 1
            FROM Employee_Roles role_filter
            WHERE role_filter.EmployeeId = e.id
                AND role_filter.RoleId IN (1, 4, 5)
    );
